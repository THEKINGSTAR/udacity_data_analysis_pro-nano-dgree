...
for year , names in nominated.items():
    for name in names:
        directores_names.append(name)


singel_list_of_names = list(set(directores_names))

#print(singel_list_of_names)
#print("#"*20)

"""CREATE DICTINORY WITH NAMES AS KEY , COUT OF EACH DIRECTOR WIN
 loop throw NOMINATED DICTINORY AND the LIST OF names with the years of the nominated add one for the  """
for dircitor_name in nominated.values():
    for name in dircitor_name:
        if name not in nom_count_dict:
            nom_count_dict[name] = 1
        elif name in nom_count_dict:
            nom_count_dict[name] += 1
...
win_count_dict = {}
# Add your code here
for dircitor_name in winners.values():
    for name in dircitor_name:
        if name not in win_count_dict:
            win_count_dict[name] = 1
        elif name in win_count_dict:
            win_count_dict[name] += 1
...
To solve this question, I use the .items method for dictionaries. Remember,
 the key in our nominated dictionary is a list of nominated directors. Think Compound Data Structures!
...
After that, if the director isn't yet in our dictionary, we give that director a count of one. 
If the director is in the dictionary, we increment that director's count by one.
 """
...
We could use the same approach as in question 1a and it would work fine, but I've provided a shorter alternative here. 
Instead of the last 4 lines as above, I've just written 1 line, by using the .get method.
